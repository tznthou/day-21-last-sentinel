...
- **Landing Page 引導**：首次訪問展示產品介紹，讓用戶了解功能後再決定註冊
...
| Swagger UI | API 文件 | 互動式文件介面 |
| Node.js Test | 單元測試 | 內建測試框架 |
...
啟動伺服器後訪問 `/api-docs` 可查看互動式 API 文件（Swagger UI）。

- **開發環境**：`http://localhost:3000/api-docs`
- **生產環境**：`https://your-domain.com/api-docs`

API 文件包含所有端點的說明、請求/回應格式，並可直接在瀏覽器中測試 API。
...
1. 開啟網站後會先看到 Landing Page 介紹
2. 點擊 `[ 進入系統 ]` 進入登入/註冊畫面
3. 點擊 `[ 使用說明 ]` 可查看操作指南
4. 已登入用戶會自動跳過 Landing Page 直接進入主控台
...
| 代碼 | 項目 | 完成日期 |
|------|------|----------|
| L03 | 單元測試 (39 個測試案例) | 2025-12-20 |
| L07 | API 文件 (Swagger UI) | 2025-12-20 |
